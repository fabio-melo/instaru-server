...
async function createOrUpdateFile(resource, rawFile, uploadFile) {
  ...
  var filenameToUse = uuidv4() + extractExtension(rawFile.name);
  console.log(filenameToUse);
  var storageRef = storageRoot.child(resource + '/' + filenameToUse);
...
*/
export const FireData = (type, resource, params) => {
  switch (type) {
      case GET_LIST: {
          ...
          return Promise
                  ...
                  .then(arrayOfResults => {
                      arrayOfResults.map( (keyAndUrl) => {
                          ...
                          params.data[keyAndUrl.key] = keyAndUrl.downloadURL;
                          return params.data
                      });
...
export const FireAuth = (type, params) => {
    if (type === AUTH_LOGIN) {
        ...
        if (!auth) {
          try {
            auth = firebase.auth().signInWithEmailAndPassword(username, password)
          } catch (error) {
